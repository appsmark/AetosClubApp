...
export class Ranking {
    ...
    rssFeed(id) {
		...
		.then((r) => {
			...
			poule = poule.replace("Jongens A ", "")
			poule = poule.replace("Jongens B ", "")
			poule = poule.replace("Jongens C ", "")
			poule = poule.replace("Meisjes A ", "")
			poule = poule.replace("Meisjes B ", "")
			poule = poule.replace("Meisjes C ", "")
			poule = poule.replace("Dames ", "")
			...
			for (var count=0; count < 10; count++) {
				...
				xml = xml.replace("Jumbo Van Andel-", "")
				xml = xml.replace("ROOT ", "")
				xml = xml.replace("Sportclub W", "W")
				xml = xml.replace("'Topklimaat in Volleybal'", "")
				xml = xml.replace("HevaV", "Heva VCV")
				xml = xml.replace("Steenderen ", "")
				xml = xml.replace("Dros-", "")
				xml = xml.replace("TiMaX/", "")
